...
export class CountryService {
  ...
  emitProduitsSubject() {
    this.countrySubject.next(this.countries.slice());
  }

  getProduitById(code: string) {
  ...
  sendBill(detail, product, delivery) {

    let bill = [detail, product, delivery]

    console.log(delivery);

    this.http
      .post(this.baseUrl + '/send-bill', bill)
      .subscribe(
        (res) => {
